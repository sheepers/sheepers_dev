...
(function ($) {
    ...
    $.widget("ui.writetable", {
        ...
        _initColumns: function() {
            ...
            headers.each(function(index) {
                ...
                self.columns.push({
                    ...
                    selectType: colSettings.selectType,
                    fromList:colSettings.fromList
                });
            });
        },
        ...
        _rowSelected:function (event) {
            ...
            // Focus any first cell in selected row
            var cellInput = $(event.target).closest('td').children().first();
        ...
        _makeRowEditable: function(row) {
            ...
            if(!row.hasClass(self.options.editModeClass)) {    //only if the row is not already editable
                row.siblings("."+self.options.editModeClass).each(function() {
                    ...
                    $(this).children("td").has("select").each(function() {
                        var select = $(this).children("select").first();
                        select.hide();
                         $(this).children("span").show().text(select.val());
                    });
                });
        ...
        _initEditableCell: function(cell, columnIndex, rowId) {
            ...
            if(input.size() == 0) {  //if the cell doesn't already have an input, create one
                ...
                var selecttype = self.columns[columnIndex].selectType;
                var fromlist = self.columns[columnIndex].fromList;
                var value = cell.text();

                if (selecttype==true) {


                    var selectCommand = "<select id='"+name+"' name='"+name+"' >";
                    fromlist = fromlist.replace("[","");
                    fromlist = fromlist.replace("]","");
                    var fromArray = fromlist.split(",");
                    for (i in fromArray){
                        selectCommand = selectCommand + " <option value='" + fromArray[i] + "' >" + fromArray[i] + " </option>";
                    }
                    selectCommand = selectCommand + "</select>";
                    input = $(selectCommand);
        ...
        _rowIsBlank: function(row) {
            ...
            row.children('td').each(function(i) {
                var td = $(this);
                if(td.text().trim() != '' || td.children().first().val()) {
                    isBlank = false;
                    return;
                }
            });
