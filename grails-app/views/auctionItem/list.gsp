			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="comments" title="${message(code: 'auctionItem.comments.label', default: 'Comments')}" />
					
						<g:sortableColumn property="amountOfBoxes" title="${message(code: 'auctionItem.amountOfBoxes.label', default: 'Amount Of Boxes')}" />
						...
						<th><g:message code="auctionItem.auction.label" default="Auction" /></th>
				...
				<g:each in="${auctionItemInstanceList}" status="i" var="auctionItemInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${auctionItemInstance.id}">${fieldValue(bean: auctionItemInstance, field: "comments")}</g:link></td>
					
						<td>${fieldValue(bean: auctionItemInstance, field: "amountOfBoxes")}</td>
					
						<td>${fieldValue(bean: auctionItemInstance, field: "size")}</td>
					
						<td>${fieldValue(bean: auctionItemInstance, field: "typeOfItem")}</td>
					
						<td>${fieldValue(bean: auctionItemInstance, field: "auction")}</td>
